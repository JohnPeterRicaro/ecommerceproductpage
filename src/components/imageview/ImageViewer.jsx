...
export const ImageButton = ({ imageData, imageKey, children }) => {
  const {
    isKey,
    isActive,
    setIsActive,
...
const ImageViewer = () => {
  ...
  const handleLeftClick = () => {
    setKey((index) => {
      return index <= 0 ? isData[isKey]?.bg.length - 1 : index - 1;
    });
  };

  const handleRightClick = () => {
    setKey((index) => {
      return index >= isData[isKey].bg.length - 1 ? 0 : index + 1;
    });
  };
      ...
      {isActive && (
        <>
          <BodyClassName className="relative w-full h-full overflow-hidden pr-[19px]">
            <section className="z-30 fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-70 backdrop-blur-[2px] flex justify-center items-center">
